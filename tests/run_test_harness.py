...
LIST_OF_TESTS = [
    ...
    ("ping_pong_equal_async", UNSPECIFIED_NUM_THREADS),
    ("ping_pong_unequal_async", UNSPECIFIED_NUM_THREADS),
    ("super_light_async", UNSPECIFIED_NUM_THREADS),
    ("super_super_light_async", UNSPECIFIED_NUM_THREADS),
    ("recursive_fibonacci_async", UNSPECIFIED_NUM_THREADS),
    ("math_operations_in_tight_for_loop_async", UNSPECIFIED_NUM_THREADS),
    ("math_operations_in_tight_for_loop_fan_in_async",
     UNSPECIFIED_NUM_THREADS),
    ("math_operations_in_tight_for_loop_reduction_tree_async",
     UNSPECIFIED_NUM_THREADS),
    ("spin_between_run_calls_async", UNSPECIFIED_NUM_THREADS),
    ("mandelbrot_chunked_async", UNSPECIFIED_NUM_THREADS),
    ("simple_run_deps", UNSPECIFIED_NUM_THREADS),
]
...
def pretty_print_with_comparison(test_name, runtimes, perf_threshold,
                                 impl_perf_ok):
    print("Results for: %s" % test_name)
    ...
    for impl in LIST_OF_IMPLEMENTATIONS:
        ...
        student_time = runtimes[
            student_impl] if student_impl in runtimes else "Missing"
        ...
        print("{:<40}{:<10}{:<12}{:.2f}  {}".format(impl, student_time,
                                                    ref_time, relative_perf,
                                                    feedback))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        help=
        "Max number of threads that the task system can use. (%d by default)" %
    ...
    parser.add_argument('-t',
                        ...
                        help='List of tests to run: %s' %
                        ", ".join([x[0] for x in LIST_OF_TESTS]))
    ...
    for (test_name, num_threads) in test_names_and_num_threads:
        ...
        if platform.system() == 'Darwin':
            ...
            if platform.machine() == "arm64":
                ...
                ref_cmd = "./%s_osx_arm -n %d" % (REFERENCE_BINARY_NAME,
                                                  num_threads)
            else:
                print("Reference binary: ./runtasks_ref_osx_x86")
                ref_cmd = "./%s_osx_x86 -n %d" % (REFERENCE_BINARY_NAME,
                                                  num_threads)
        else:
        ...
        pretty_print_with_comparison(test_name, all_runtimes, PERF_THRESHOLD,
                                     impl_perf_ok)
    ...
    for impl in LIST_OF_IMPLEMENTATIONS:
        final_feedback = "All passed Perf" if impl_perf_ok[
            impl] else "Perf did not pass all tests"
